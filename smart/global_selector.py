...
def train(args):
    ...
    print("Starting loadifng features...")
    ...
    global_frame_selector = MLP(10).to(device)
    ...
    best_model_path = '/data4/zst/uav/smart/data/best_global_frame_selector.pt'
    ...
    for epoch in range(args.epochs):
        ...
        for train_feature, train_labels in tqdm(train_loader, desc=f'Epoch {epoch+1}'):
            ...
            seq_len = train_feature.size(1)
            sum_frame += seq_len
            train_labels = repeat(train_labels, 'b -> b s', s=seq_len).to(device)
        ...
        with torch.no_grad():
            for valid_feature, valid_labels in tqdm(val_loader):
                ...
                seq_len = valid_feature.size(1)
                sum_frame += seq_len
                valid_labels = repeat(
                    valid_labels, 'b -> b s', s=seq_len).to(device)
        ...
        acc = accuracy_score(y_true=y_true, y_pred=y_pred)
        print(
            f"         valid loss: {avg_loss:.4f}, acc: {acc:.4f}")
