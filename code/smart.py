...
from single_selector import SingleFrameSelector, load_singleframeselector
from global_selector import GlobalSelector,load_globalselector
...
class SmartModel(nn.Module):
    def __init__(self, num_classes):
        ...
        self.single_frame_selector=load_singleframeselector(model_path="models/single_frame_selector",num_features=1280)
        self.global_selector=load_globalselector(model_path="models/global_frame_selector",num_features=1280,num_classes=10)
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            features = features.view(features.size(0), features.size(1), -1)  # 调整特征形状以匹配选择器的输出
        # 使用SingleFrameSelector选择帧
        selected_frame_features = self.single_frame_selector(features)
        
        # 使用GlobalSelector进行最终的分类决策
        global_select_result = self.global_selector.get_frame_importance(features)
        #使用final_selector来综合上述结果,从而生成最终选帧
        predictions=self.final_frame_selector(selected_frame_features,global_select_result)
    ...
    def get_selected_frame(self, predictions:torch.Tensor, x:torch.Tensor)->torch.Tensor:
        ...
        selected_feature=rearrange(selected_feature,"(b s) f -> b s f",b=batch_size)
